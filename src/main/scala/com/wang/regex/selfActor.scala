 ...
 *
 * https://github.com/scalaz/scalaz/blob/scalaz-seven/concurrent/src/main/scala/scalaz/concurrent/selfselfActor.scala
 *
...
/**
  ...
  * the `selfActor` reads a volatile memory location before entering its event loop, and writes to the same
  ...
  */
final class selfActor[A](strategy: Strategy)(handler: A => Unit, onError: Throwable => Unit = throw(_)) {
  self =>
  ...
  def contramap[B](f: B => A): selfActor[B] =
    new selfActor[B](strategy)((b: B) => (this ! f(b)), onError)
...
object selfActor {
  ...
  /** Create an `selfActor` backed by the given `ExecutorService`. */
  def apply[A](es: ExecutorService)(handler: A => Unit, onError: Throwable => Unit = throw(_)): selfActor[A] =
    new selfActor(Strategy.fromExecutorService(es))(handler, onError)
}
